...
struct PLAYER_NAME : public Player {
    ...
    void bfs(int x, int y, int id, int RANGE, map<int, Dir>& commands) {
            ...
            if (is_food(i, j) and dist[i][j] < minim_food) {
                if (being_taken[i][j].F == -1) {
                    ...
                    if (being_taken[i][j].S > dist[i][j]) {
                        ...
                        bfs(unit(id_r).pos.i, unit(id_r).pos.j, id_r, RANGE, commands);
                        PQ.PB(MP(0, node));
                    }
            ...
            if (is_zombie(i, j) and dia_dist[i][j] <= 2) {
                ...
                if (adjacent_to_zombie(x, y)) {
                    ...
                    for (int d = 0; d < 4; ++d) {
                        int ni = x + idir[rand[d]];
                        int nj = y + jdir[rand[d]];
                        if (posok(ni, nj) and not is_ally(ni, nj) and not is_enemy(ni, nj) and not adjacent_to_zombie_in_dia(ni, nj)) {
                            commands[id] = dirs[rand[d]];
                            return;
                        }
                    }
